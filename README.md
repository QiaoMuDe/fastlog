...
基于实际测试数据（22核CPU，10个Goroutine并发）：
...
| 指标类型 | 数值 | 说明 |
|---------|------|------|
| **吞吐量** | 68.8万条/秒 | 10个Goroutine并发下的实际处理能力 |
| **处理延迟** | 1.45μs | 平均每条日志的处理时间 |
| **内存效率** | 7.61 bytes/条 | 平均每条日志的内存开销 |
| **智能背压** | 2.45% | 高负载下的日志保留率（智能丢弃97.55%） |
| **GC友好** | 39次GC | 1.3秒内仅触发39次垃圾回收 |
| **内存峰值** | 39.5MB | 处理90万条日志的最大内存占用 |
...
- **文件输出**: 启用（实际写入2.2万条有效日志）

### 性能特点

1. **高吞吐量**: 在10个Goroutine并发下达到68.8万条/秒的处理能力
2. **低延迟**: 平均1.45微秒的单条日志处理时间
3. **内存高效**: 每条日志仅占用7.61字节内存开销
4. **智能背压**: 高负载下自动保留重要日志，丢弃冗余信息
5. **GC友好**: 优化的内存管理减少垃圾回收压力
