...
type FastLogConfig struct {
	...
	StringPoolMaxCapacity int           // 字符串池最大容量，默认 512
}
...
func NewFastLogConfig(logDirName string, logFileName string) *FastLogConfig {
	...
	return &FastLogConfig{
		...
		StringPoolMaxCapacity: 512,                    // 字符串池最大容量，默认 512
	}
}
...
func (c *FastLogConfig) fixFinalConfig() {
	...
	// 10.验证字符串池容量
	if c.StringPoolMaxCapacity <= 0 {
		c.StringPoolMaxCapacity = 512 // 默认512
	} else if c.StringPoolMaxCapacity > 100000 {
		c.StringPoolMaxCapacity = 100000 // 最大限制
	}
}
