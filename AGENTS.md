...
├── .gitignore          # Git 忽略规则
│
├── logger.go           # Level + Entry + Logger 实现 + EntryPool（原 fastlog.go 合入）
├── logger_test.go      # Logger 单元测试（大幅扩充）
├── config.go           # Config 配置结构体 + 6 种场景化配置函数
├── config_test.go      # Config 单元测试（新增）
├── field.go            # 字段类型定义与构造函数
├── field_test.go       # Field 单元测试（大幅扩充）
├── formatter.go        # Formatter 接口 + 5 种内置格式（原 fastlog.go 接口合入）
├── formatter_test.go   # Formatter 单元测试（大幅扩充）
...
| `fastlog_test.go` | ~92 | Level 基础类型单元测试（String/Enabled/ParseLevel/AllLevels） | ✅ 覆盖完善 |
| `http.go` | ~60 | HTTP 请求日志中间件（输出含 method/path/status/duration/remote_addr 等） | ✅ 轻量实用 |
| `http_test.go` | ~73 | HTTP 中间件测试（含多组模拟请求） | ⚠️ 无断言（视觉验证） |
...
- **优点**：
  - 文件命名清晰，按功能划分
  - 核心逻辑与示例分离
  - 示例分四个独立子目录（basic / color / sampler / formats）
  - 单元测试覆盖完善（52 个测试用例）
- **变更记录**：
...
- **配置方法**：
  - `Validate()` - 配置验证（含采样器、文件路径、缓冲区、同步间隔等完整校验）
...
- `NewSampler` 兜底逻辑、`DefaultSampler()`、`NewConfig()` 三处统一引用常量，一处修改全部同步
